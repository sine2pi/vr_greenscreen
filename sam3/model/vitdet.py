...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.drop1(x)
        x = self.norm(x)
        # addmm_act emits bf16 for performance; align to fc2 param dtype
        # to avoid matmul dtype mismatch when autocast is not active.
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x + self.dropout(self.drop_path(self.ls2(self.mlp(self.norm2(x.float())))))
